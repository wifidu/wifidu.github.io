...
> 问题关键：因为有无限件物品，所以在`0-1背包`里`f[i - 1][j - weight[i]] + value[i])`是在`i - 1`件物品时最大组合，
> 而我们现在第`i`件有n件，所以我们需要用第`i`行的`j - weight`时候的最大组合:`f[i][j - weight[i]] + value[i])`
...
写成一维形式之后和01背包完全相同，唯一的区别在于这里v的枚举顺序是正向枚举，而01背包的一维形式中v必须是逆向枚举。完全背包的一维形式代码如下所示：
...
这题目和完全背包问题很像。基本的方程只需将完全背包问题的方程略微一改即可，因为对于第i种物品有n[i]+1种策略：取0件，取1件，取2件…取n[i]件。令f[i][v]表示前i种物品恰放入一个容量为v的背包的最大权值。
